...
def insert_sensor_data2(smoke, temperature, fire):
    create_table2()  # Ensure table exists
    try:
        cursor.execute("""
        INSERT INTO main_table (smoke, temperature, fire) 
        VALUES (?, ?, ?);
        """, (smoke, temperature, fire))

        conn.commit()
        print(f"Data inserted into main_table: Smoke={smoke}, Temperature={temperature}, Fire={fire}")

    except Exception as e:
        print(f"Error inserting data into main_table: {e}")
...
###################### For delete data from temp table  ###############################
def DeleteOldestRecord():
    """Delete the oldest record from the sensor_data table to keep data manageable."""
    try:
        conn = conn
        cursor = conn.cursor()
        delete_query = """  
        DELETE FROM sensor_data 
        WHERE id = (SELECT id FROM (SELECT id FROM sensor_data ORDER BY id ASC LIMIT 1) AS subquery);
        """
        ...
        print("deleting data error resan : ", e)
###################### For delete data from temp table  ###############################
...
def get_sensor_data2():
    ...
    try:
        with sqlite3.connect(DB_NAME) as conn:  
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM main_table ORDER BY id DESC")  
            data = cursor.fetchall()  
            cursor.close()
